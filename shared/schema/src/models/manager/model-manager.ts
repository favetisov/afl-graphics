import { getLastUpdate, getUpdatesSince, loadItem, loadList, newsPlain, recommendationsPlain, searchPlain } from './manager.client';
...
export class ModelManager {
  ...
  async getModelsList(entities: Array<{ entityName: string; entityId: number }>) {
    await this.getUpdates();
  ...
  private async unpackDb() {
    await Promise.all(Object.keys(schema).map(async key => (this.data[key + '_plain'] = await this.db[key].toArray())));
  ...
  private plainToData(eName, row) {
    ...
    let i = 0;
    [...(schema[eName].fields.list || []), ...(schema[eName].fields.public || []), ...(schema[eName].fields.admin || [])].forEach(field => {
      ...
      if (parentId) {
        ...
        if (field) {
          ...
          if (!this.data[parentName][parentId + ''][field + '_map'][_id]) {
            this.data[parentName][parentId + ''][field].push(this.data[eName][_id]);
          }
  ...
  private async loadMissingEntities(entities: { [eName: string]: Array<string | number> }) {
    const entitiesToLoad = {};
    for (const eName in entities) {
      entitiesToLoad[eName] = entities[eName].filter(_id => !this.data[eName][_id]?._id);
    }

    if (!Object.keys(entitiesToLoad).some(e => entitiesToLoad[e].length)) return;
    ...
    Object.keys(entitiesToPut).map(eName => this.db[eName].bulkPut(entitiesToPut[eName]));
  }
}
