...
export const PhotoLoader = ({
  ...
  useEffect(() => {
    if (!defaultInitiated && defaultUrl && canvasEl?.current) {
      ...
      (async () => {
        ...
        image.onload = () => {
          const { width: canvasWidth, height: canvasHeight } = canvasEl.current.getBoundingClientRect();
  ...
  return (
    <div className={`${s.loader} ${className}`} onMouseOver={() => setShowCloseButton(true)} onMouseOut={() => setShowCloseButton(false)}>
      <LoadingOverlay visible={loading} />
      {photo && showCloseButton && (
        <Button onClick={() => setPhoto(null)} className={s.closeButton + ' photoloader-button'}>
          <CloseIcon size={14} />
        </Button>
      )}
      {!photo && <input type={'file'} accept=".jpg, .jpeg, .png" onChange={e => onImgLoad(e)} />}
      {!photo && <PhotoIcon size={100} className={s.photoIcon} />}
      {photo && (
        <TransformWrapper onInit={drawOnCanvas} onZoomStop={drawOnCanvas} onPanningStop={drawOnCanvas} minScale={0.1} limitToBounds={false}>
          <TransformComponent>
            <img src={photo} height={height} width={width} style={{ objectFit: 'contain' }} />
          </TransformComponent>
        </TransformWrapper>
      )}
      <canvas ref={canvasEl} className={s.canvas} height={height} width={width} />
    </div>
  );
};
