...
export const IgraphicsQuadraticCupGameRow = forwardRef(
  ({ game, schema, teamLogo, teamName, teamScore, team }: any, ref) => {
    ...
    if (!team) {
      return (
        <div
          className={s.team}
          style={{ backgroundColor: schema.colors.rowOdd }}
        />
      );
    }
      ...
      <div className={s.team} style={{ backgroundColor: schema.colors.rowOdd }}>
        ...
        {scoreResult.map((sc, idx) => (
          <div
            key={idx}
            ref={(el) =>
              el?.style?.setProperty("--borderColor", schema.colors.rowFont)
            }
            style={{ color: schema.colors.rowFont }}
            className={s.score}
          >
            <input
              style={{ color: schema.colors.rowFont }}
              onChange={(e) => onChangeScore(e.target.value, sc, idx, "ft")}
              value={sc.ft}
            />
            {sc.hasPen && (
              <input
                style={{ color: schema.colors.rowFont }}
                onChange={(e) => onChangeScore(e.target.value, sc, idx, "pen")}
                value={sc.pen}
                className={s.scorePen}
              />
            )}
          </div>
        ))}
