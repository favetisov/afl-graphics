...
export const IgraphicsMvpPlayer2Component = forwardRef(({ league, schema, pattern, mode }: any, ref) => {
    ...
    <div className={s.tableWrapper}>
      ...
      <div className={s.content}>
        <div className={s.playerPhoto}>
          <div>
            <div ref={el => el?.style?.setProperty('--background-loader', schema.colors.bg)} className={s.photo}>
              <PhotoLoader className={s.loaderPhoto} defaultUrl={'/igraphics/demo/messi.jpg'} onImg={() => {}} height={2500} width={2500} />
            </div>
        ...
        <div style={{ background: schema.colors.cardWrapperBg }} className={s.info}>
          ...
          <div className={s.name}>
            <input style={{ color: schema.colors.rowFont }} className={s.team} value={teamName} onChange={e => setTeamName(e.target.value)} />
            <input style={{ color: schema.colors.rowFont }} className={s.player} value={playerName} onChange={e => setPlayerName(e.target.value)} />
          </div>
        </div>
      </div>
