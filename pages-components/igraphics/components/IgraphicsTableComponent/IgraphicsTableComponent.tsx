...
export const IgraphicsTableComponent = forwardRef(({ season, schema, pattern }: { season: Season; schema: any; pattern: any }, ref) => {
  if (!season) return;
  if (!schema) return;

  const renderStage = (stage: Stage) => {
    if (stage.format == StageFormat.league) {
      return <IgrStageTable stage={stage} schema={schema} key={'table' + stage._id} />;
    }
  };
  ...
  return (
    <div className={s.tableWrapper}>
      <IgrCardWrapper schema={schema}>
        ...
        <IgrTitle title={'Таблица'} schema={schema} key={'ttl' + season._id}></IgrTitle>
