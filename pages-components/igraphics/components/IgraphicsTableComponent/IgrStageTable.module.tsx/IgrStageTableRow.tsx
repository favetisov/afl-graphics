...
export const IgrStageTableRow = forwardRef(({ row, idx, schema }: any, ref) => {
  ...
  const [goalsDiff, setGoalsDiff] = useState(row.scored - row.conceded > 0 ? `+${row.scored - row.conceded}` : row.scored - row.conceded);
  ...
  return (
    <div className={hidden ? s.hidden : ''}>
      <IgrElContainer onClose={() => setHidden(true)}>
        <div className={s.tr} key={row.team._id} style={{ backgroundColor: idx % 2 ? schema.colors.rowEven : schema.colors.rowOdd }}>
          <div className={`${s.td} ${s.position}`}>
            <input style={{ color: schema.colors.rowFont }} value={position} onChange={e => setPosition(e.target.value)} />
          </div>
          <div className={`${s.td} ${s.name}`}>
            <div>
              <IgrTeamLogo height={90} logo={row.team.logo} /> <input style={{ color: schema.colors.rowFont }} value={teamName} onChange={e => setTeamName(e.target.value)} />
            </div>
          </div>
          <div className={`${s.td} ${s.gms}`}>
            <input style={{ color: schema.colors.rowFont }} onChange={e => setPlayed(e.target.value)} value={played} />
          </div>
          <div className={`${s.td} ${s.wdl}`}>
            <input style={{ color: schema.colors.rowFont }} onChange={e => setWdl(e.target.value)} value={wdl} />
          </div>
          <div className={`${s.td} ${s.goals}`}>
            <input style={{ color: schema.colors.rowFont }} onChange={e => setGoals(e.target.value)} value={goals} />
            <input style={{ color: schema.colors.rowFont }} className={s.goalsDiff} onChange={e => setGoalsDiff(e.target.value)} value={goalsDiff} />
          </div>
          <div className={`${s.td} ${s.points}`}>
            <input style={{ color: schema.colors.rowFont }} onChange={e => setPoints(e.target.value)} value={points} />
          </div>
          <div className={`${s.td} ${s.form}`}>
            <div>
              {row.games
                ...
                .slice(-5)
                .map(g => (
                  <div key={g._id} className={s.formIndicator} style={{ backgroundColor: getFormColor(row.team, g) }} />
