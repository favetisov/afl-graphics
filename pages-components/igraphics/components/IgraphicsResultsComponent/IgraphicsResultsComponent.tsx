...
export const IgraphicsResultsComponent = forwardRef(({ season, schema, pattern, fromDate, toDate, mode }: any, ref) => {
  if (!season) return;
  if (!schema) return;
  ...
  const games = season.games.filter(g => g.dt > fromDate && g.dt < toDate.add(1, 'd'));

  const gamesByDate = groupBy(games, g => g.dt.locale(userState.language).format('DD-MM-YYYY'));
  ...
  return (
    <div className={s.tableWrapper}>
      <IgrCardWrapper schema={schema}>
        ...
        <IgrSubtitle first={season.champ.name} second={season.name} schema={schema} key={'sub' + season._id} />
        <IgrTitle title={mode == 'results' ? 'результаты' : 'расписание'} schema={schema} key={'ttl' + mode + season._id}></IgrTitle>
        <div className={s.day}>
          {sortBy(Object.values(results), [d => d[0].dt]).map((games, idx) => (
            <IgrDayGames key={idx} games={games} schema={schema} mode={mode} />
          ))}
        </div>
