...
export const IgraphicsPlayersComponent = forwardRef(({ season, schema, pattern, mode }: any, ref) => {
    ...
    mode == 'points'
      ? {
          ...
          assists: orderBy(players, ['assists', 'games'], ['desc', 'asc'])
            ...
            .slice(0, 10),
          goals_assists: orderBy(players, ['goals_assists', 'games'], ['desc', 'asc'])
  ...
  return (
    <div className={s.tableWrapper}>
      ...
      <IgrTitle light title={mode == 'points' ? 'лучшие игроки' : 'карточки'} schema={schema} key={'ttl' + mode + season._id} />
            ...
            <IgrElContainer onClose={() => hideCategory(cat)}>
              <div className={s.cardWrapper} key={cat}>
                <div className={s.tableHead}>
                  <input className={s.inputTitle} defaultValue={catLabel(cat)} />
                </div>
                <div className={s.card}>
                  {categories[cat].map((row, idx) => {
                    return <IgraphicsPlayerRow row={row} schema={schema} category={cat} key={idx} />;
                  })}
                </div>
              </div>
            </IgrElContainer>
