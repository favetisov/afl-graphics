...
import { Button, ColorInput, Divider, Group, Loader, Select, Text, Title } from '@mantine/core';
...
export const IgraphicsVideoPage = () => {
  ...
  const [homeColor, setHomeColor] = useState<string>(game?.home?.team?.color || '#E73C28');
  const [awayColor, setAwayColor] = useState<string>(game?.away?.team?.color || '#1145FF');
    ...
    reader.onloadend = function () {
      if (currentComponent.value != 'events') {
        ...
        image.onload = () => {
          ...
          const events = [...containerRef.current?.querySelectorAll('.event-row')];
          ...
          const scale = 1 - Math.abs((resolutions[resolution].size.width / 5000) * 100 - 100) / 100;

          ctxEvent.scale(scale, scale);

          events.forEach((el, idx) => {
            ctxEvent.clearRect(0, 0, 5000, 5000);
            ctxEvent.drawImage(canvas, 0, TOP_Y, 1300, SCORE_HEIGHT, 50, 50, 1300, SCORE_HEIGHT);

            if (idx % 2 !== 0) {
              ctxEvent.drawImage(canvas, 1300, TOP_Y, 2000, EVENT_HEIGHT, resolutions[resolution].playerPosition.dx, resolutions[resolution].playerPosition.dy, 2000, EVENT_HEIGHT);
            }

            const height = idx % 2 == 0 ? SCORE_HEIGHT : EVENT_HEIGHT;
            TOP_Y += height + MARGIN_EVENT;

            zip.file(`event-${idx + 1}.png`, urlToPromise(canvasEvent.toDataURL()) as any, { base64: true });
          });
    ...
    if (currentComponent.value == 'cover') {
        ...
        <>
          ...
          <div ref={containerRef}>
            <div ref={el => scaleContainer(el, 4000)} className={s.imgContainer}>
              <IgraphicsCoverComponent pattern={pattern} league={league} key={game._id} game={game} schema={schema} />
            </div>
          </div>
        </>
      );
      ...
      return (
        <>
          <Divider variant={'dashed'} my={'md'} />
          <div ref={containerRef}>
            <div ref={el => scaleContainer(el, 4500)} className={s.imgContainer}>
              <IgraphicsCover2Component pattern={pattern} schema={schema} game={game} key={game._id} />
            </div>
          </div>
        </>
      );
      ...
      return (
        <>
          ...
          <div ref={containerRef}>
            <div ref={el => scaleContainer(el, 4500)} className={s.imgContainer}>
              <IgraphicsCoverPhotoComponent pattern={pattern} schema={schema} game={game} key={game._id} />
            </div>
          </div>
        </>
      );
      ...
      return (
        <>
          ...
          <div ref={containerRef}>
            <div ref={el => scaleContainer(el, 4000)} className={s.imgContainer}>
              <IgraphicsVideoResultComponent schema={schema} game={game} key={game._id} />
            </div>
          </div>
        </>
      );
        ...
        <>
          ...
          <div>
            <Group>
              ...
              <Select data={Object.keys(resolutions)} onChange={v => setResolution(v)} value={resolution} size={'xs'} />
            </Group>
          </div>
          <Divider variant={'dashed'} my={'md'} />
          <Group>
          ...
          <div ref={containerRef}>
            <div ref={el => scaleContainer(el, 4000)} className={s.imgContainer}>
              <IgraphicsEventsComponent homeColor={homeColor} awayColor={awayColor} schema={schema} game={game} key={game._id + (schema as any)?.value} />
            </div>
          </div>
        </>
      );
      ...
      return (
        <>
          ...
          <div ref={containerRef}>
            <div ref={el => scaleContainer(el, 4000)} className={s.imgContainer}>
              <IgraphicsScoreboard pattern={pattern} schema={schema} game={game} homeColor={homeColor} awayColor={awayColor} key={game._id} />
            </div>
          </div>
        </>
      );
      ...
      return (
        <div ref={containerRef}>
          <div ref={el => scaleContainer(el, 4000)} className={s.imgContainer}>
            <IgraphicsStatsPlayers pattern={pattern} schema={schema} game={game} key={game._id} />
          </div>
        </div>
      );
      ...
      return (
        <div ref={containerRef}>
          <div ref={el => scaleContainer(el, 4000)} className={s.imgContainer}>
            <IgraphicsVideoResultPhotoComponent pattern={pattern} schema={schema} game={game} key={game._id} />
          </div>
        </div>
      );
    }
  }, [currentComponent?.value, game?._id, homeColor, awayColor, (schema as any)?.value, pattern, resolution]);
    ...
    return (
      <div>
        ...
        <Select
          ...
          data={season?.games.map(g => ({
            value: g._id,
            label: g.home.team.name + ' - ' + g.away.team.name + ', ' + g.tourNumber + ' round',
            game: g,
          }))}
          size={'xs'}
          mt={'xs'}
          ...
          filter={(value, item) => {
            const parts = value.trim().toLowerCase().replace(/\s\s+/g, ' ').split(' ');
            const values = [item.game.home.team.name.toLowerCase(), item.game.away.team.name.toLowerCase(), item.game.tourNumber + ''];
    ...
    <>
      <Title order={2} my={'sm'}>
        <Group position={'apart'}>
          Igraphics video
          <Button mt={'md'} onClick={() => exportAsImage(currentComponent.value + '.png')} disabled={downloading}>
            DOWNLOAD IMAGE
