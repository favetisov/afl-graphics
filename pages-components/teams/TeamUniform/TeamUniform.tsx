...
import { X as CloseIcon, Plus as PlusIcon } from 'tabler-icons-react';
import s from './TeamUniform.module.scss';
import { TeamUniformEditModal } from './TeamUniformEditModal/TeamUniformEditModal';
export type UniformType = { shirt_left: string; shirt_right: string; shorts: string; socks: string };

export const TeamUniform = ({ team, onFormSet }: { team?: Team; onFormSet: (u: UniformType[]) => any }) => {
  const [uniforms, setUniforms] = useState<UniformType[]>([]);
  const [currentIndex, setCurrentIndex] = useState<number>(null);

  const removeUniform = (index: number) => {
    setUniforms(prev => prev.filter((u, idx) => idx != index));
    onFormSet(uniforms);
  };

  const onSaveForm = (uniform: UniformType) => {
    uniforms[currentIndex] = uniform;
    setUniforms([...uniforms]);
    setCurrentIndex(null);
    onFormSet(uniforms);
  };

  if (!uniforms.length && team?.uniform?.length) {
    setUniforms(team.uniform);
  }
    ...
    <div>
      <Group spacing={'sm'}>
        {uniforms?.map((uniform, idx) => (
          <Paper className={s.uniform} key={idx}>
            <svg onClick={() => setCurrentIndex(idx)} stroke={'#c1c1c1'} width="60" height="70" viewBox="0 0 83 128" xmlns="http://www.w3.org/2000/svg">
              <g fill={uniform.shirt_left}>
              ...
              <g fill={uniform.socks} clipPath="url(#clip0_2030_8691)">
            ...
            <Button onClick={() => removeUniform(idx)} variant="filled" color="gray" className={s.closeIconContainer}>
              <CloseIcon size={13} className={s.closeIcon} />
            </Button>
          </Paper>
        ))}
        <Paper className={s.newUniformContainer} onClick={() => setCurrentIndex(uniforms.length)}>
          <svg stroke={'#c1c1c1'} width="60" height="70" viewBox="0 0 83 128" xmlns="http://www.w3.org/2000/svg">
            <g fill="#fff">
              <path d="M73.5536 17.8267C74.0723 16.8263 73.6626 15.5952 72.648 15.1051L52.0246 5.14405C51.9608 5.12179 51.8929 5.11383 51.8257 5.12072C51.7585 5.12761 51.6936 5.1492 51.6357 5.18394C51.5778 5.21868 51.5282 5.26573 51.4904 5.32177C51.4527 5.3778 51.4277 5.44144 51.4173 5.50819C50.9863 8.09067 49.6532 10.4366 47.6553 12.1287C45.6573 13.8208 43.6194 15 41.0012 15L41 55.3125H57.0949C57.3896 55.3162 57.6814 55.2552 57.95 55.1339C58.273 54.9797 58.5442 54.7348 58.7304 54.4291C58.9166 54.1234 59.0098 53.7701 58.9986 53.4123L58.1539 25.6523C58.147 25.421 58.2272 25.1956 58.3788 25.0208C58.5304 24.8459 58.7422 24.7345 58.9721 24.7085L65.6977 28.487C66.6836 29.0409 67.9322 28.6679 68.4528 27.664L73.5536 17.8267Z" />
            </g>
            ...
            <g fill="#fff" clipPath="url(#clip0_2030_8691)">
              <path d="M59.9159 96.8096C61.0642 96.8096 61.9768 97.7744 61.9128 98.921L60.4605 124.958C60.4014 126.017 59.5249 126.846 58.4636 126.846L50.557 126.846C49.4525 126.846 48.557 125.951 48.557 124.846L48.5571 98.8096C48.5571 97.705 49.4525 96.8096 50.557 96.8096L59.9159 96.8096Z" />
              <path d="M22.0607 96.8096C20.9123 96.8096 19.9998 97.7744 20.0638 98.921L21.516 124.958C21.5751 126.017 22.4516 126.846 23.5129 126.846L31.4195 126.846C32.5241 126.846 33.4195 125.951 33.4195 124.846L33.4195 98.8096C33.4195 97.705 32.5241 96.8096 31.4195 96.8096L22.0607 96.8096Z" />
            </g>
            <defs>
          ...
          <Button radius={'xl'} variant="filled" color="gray" className={s.newUniformButton}>
            <PlusIcon size={16} />
          </Button>
        </Paper>
        <Modal withCloseButton={false} centered opened={currentIndex != null} onClose={() => setCurrentIndex(null)}>
          <TeamUniformEditModal onSaveForm={onSaveForm} uniformInitial={uniforms[currentIndex]} />
        </Modal>
      </Group>
    </div>
  );
